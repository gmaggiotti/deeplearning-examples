...
def load_text_processor(fname='title_pp.dpkl'):
    ...
    print('Size of vocabulary for {fname}: {num_tokens:,}')
...
def load_decoder_inputs(decoder_np_vecs='train_title_vecs.npy'):
    ...
    print('Shape of decoder target: {decoder_target_data.shape}')
...
class Seq2Seq_Inference(object):
    ...
    def generate_issue_title(self,
        ...
        raw_tokenized = np.array([12,153,34,16,17,12,153,34,16,17,12,153,34,16,17,12,153,34,16,17,12,153,34,16,17,12])  # Body text
        raw_tokenized = np.array([raw_tokenized,raw_tokenized])
        body_encoding = self.encoder_model.predict(raw_tokenized)
        # we want to save the encoder's embedding before its updated by decoder
        #   because we can use that as an embedding for other tasks.
        original_body_encoding = body_encoding
        state_value = np.array([1]) #np.array(self.pp_title.token2id['_start_']).reshape(1, 1)
        ...
        while not stop_condition:
            ...
            pred_word_str = self.idx2word[pred_idx]
    ...
    def demo_model_predictions(self,
        ...
        url, body_text, title_text = issue_df[10:20].reshape( (-1,10) )
