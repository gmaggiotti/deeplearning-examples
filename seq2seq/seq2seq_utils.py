...
class Seq2Seq_Inference(object):
    def __init__(self,
        ...
        self.default_max_len_title = len(self.pp_title[0])
        self.nn = None
        self.rec_df = None
    ...
    def generate_issue_title(self,
        ...
        body_encoding = raw_input_text
        # we want to save the encoder's embedding before its updated by decoder
        #   because we can use that as an embedding for other tasks.
        original_body_encoding = body_encoding
        state_value = np.array(3).reshape(1,1)#self.pp_title.token2id['_start_']).reshape(1, 1)
    ...
    def demo_model_predictions(self,
        ...
        demo_list = np.random.randint(low=1, high=len(issue_df), size=n)
        for i in demo_list:
            self.print_example(i,
                               body_text=issue_df[i],
                               title_text="",
                               url="",
                               threshold=threshold)
